...
// the default USB Dongle cofiguration below uses P0.2 for SCL and P0.3 for SDA.
// the default KeyFob cofiguration below uses P0.6 for SCL and P0.7 for SDA.
...
uint8 hali2cStart( void )
{
  ...
  // set SCL to input but with pull-up.
  // if slave is pulling down it will stay down.
  hali2cClock(1);
...
/*********************************************************************
 * @fn      I2C_ReadMultByte
 * @brief   writes data into a buffer
 * @param   address: linear address on part from which to read
...
uint8 I2C_ReadMultByte( uint8 address, uint16 reg, uint8 *buffer, uint16 len )
{
  ...
  if ( !hali2cSendByte(reg >> 8))
    return I2C_ERROR;
  if ( !hali2cSendByte(reg & 0xFF))
    return I2C_ERROR;
  ...
  while ( len )
  {
    ...
    buffer[len - 1] = hali2cReceiveByte();
...
uint8 I2C_WriteMultByte( uint8 address, uint16 reg, uint8 *buffer, uint16 len )
{
  ...
  /* Send internal register to write from to */
  if ( !hali2cSendByte(reg >> 8))
    return I2C_ERROR;
  if ( !hali2cSendByte(reg & 0xFF))
    return I2C_ERROR;
  ...
  while ( len )
  {
    ...
    /* Send Address  and get acknowledgement from slave*/
    if ( !hali2cSendByte(buffer[len - 1]))
      return I2C_ERROR;
...
{
  return I2C_WriteMultByte(deviceAddr, reg, (uint8 *)&value, 1);
}

uint8 I2C_WriteReg16(uint8 deviceAddr, uint16 reg, uint16 value)
{
  return I2C_WriteMultByte(deviceAddr, reg, (uint8 *)&value, 2);
}

uint8 I2C_WriteReg32(uint8 deviceAddr, uint16 reg, uint32 value)
{
  return I2C_WriteMultByte(deviceAddr, reg, (uint8 *)&value, 4);
}

uint8 I2C_ReadReg8(uint8 deviceAddr, uint16 reg)
{
  uint8 value = 0;
  I2C_ReadMultByte(deviceAddr, reg, (uint8 *)&value, 1);
  return value;
}
...
{
  uint16 value = 0;
  I2C_ReadMultByte(deviceAddr, reg, (uint8 *)&value, 2);
  return value;
}
...
{
  uint32 value = 0;
  I2C_ReadMultByte(deviceAddr, reg, (uint8 *)&value, 4);
  return value;
}
